...
from .services import GetOpinionsProduct, ProductsPageServices, DeleteOpinionServices, IndexToMove, \
    SaveOpinionServices, ChangerCountLikesServices, FinalizeServices
...
class Product(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            get_liked_opinion = ProductsPageServices(username, opinions, self.queryset[0], True)
        ...
        context['opinions'] = opinions[0:3]  # выводим только три отзыва на странице продукта
...
class OpinionsProduct(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            get_liked_opinions = ProductsPageServices(username,
                                                      self.queryset,
                                                      product,
                                                      False)
...
class SaveOpinion(View):
    def post(self, request):
        ...
        if self.request.user.is_authenticated:
            ...
            save_opinion_at_product = SaveOpinionServices(username, int(id_product), opinion)
...
class ChangerCountLikeOpinion(View):
    def post(self, request):
        if self.request.user.is_authenticated:
            ...
            update_likes = ChangerCountLikesServices(username, int(product_id), int(opinion_id))
