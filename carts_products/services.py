...
class GetOpinionsProduct:
    ...
    def get_opinions_product(self) -> List:
        opinions = Opinions.objects.filter(id_product=self.id_product).select_related('id_product', 'user')
...
class IndexToMove:
    ...
    def index_to_move_opinions(self):
        ...
        index_to_move = next((i for i, item in enumerate(self.opinions) if item.user == self.user), None)
    ...
    def get_liked_opinions(self):
        if self.opinions:
            ...
            for object_opinion in self.opinions:
                is_liked_opinion = LikesOpinion.objects.filter(user=user,
        ...
        for key, value in data_form_finalize.items():
            try:
                if not hasattr(user, value):
                    raise AttributeNotInObject(value, user)

                attribute_value = getattr(user, value)

                # Check for TypeError
                if not isinstance(user, get_user_model()):
                    raise ObjectNotValid(user)

                # Check for ValueError
                if attribute_value is None:
                    raise AttributeNotValid(user, value)
